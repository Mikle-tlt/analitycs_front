...
import {
  useOfflineGrowthStore
} from '../../../services/stores/offline-analytics/growth-analytics/offline-growth-analytics';
...
export const OfflineGrowthAnalytics = () => {
  const { error } = useAlert();
  const { revenues, days, labelText, getOfflineGrowth, setDefaultData } = useOfflineGrowthStore();
  ...
  const onGetOfflineGrowth = onHandler(async ({ productId, withDate, byDate }) => {
    const withDateForm = withDate.format("YYYY-MM-DD");
    const byDateForm = byDate.format("YYYY-MM-DD");
    await getOfflineGrowth(productId, withDateForm, byDateForm)
  }, (e) => error(e));
  ...
  return (
    <PageWrapper vertical>
      <Title level={2}>Анализ темпов оффлайн продаж</Title>
      <FilterGrowthForm submitAction={onGetOfflineGrowth} cancelAction={resetDate} />
      <Divider />
      {
        days && revenues && (
          <Chart
            ...
            options={{
              title: 'График темпов оффлайн продаж',
              hAxis: { title: 'Дата' },
