...
export const useOfflineSalesByCategoryAnalytics = create((set) => ({
  salesData: null,
  labelText: '',
  isLoading: false,

  getOfflineSalesByCategory: withLoadingStateHandling(
    async (categoryId, year) => {
      const response = await axiosPrivate.get(
        `${Urls.ANALYTICS_OFFLINE_BY_CATEGORY}/${categoryId}/${year}`);
      await set({
