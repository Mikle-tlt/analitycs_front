...
export const useOfflineProfitabilityStore = create((set) => ({
  profitabilityData: [],
  labelText: '',
  isLoading: false,

  getOfflineProfitability: withLoadingStateHandling(
    async (dateWithF, dateByF, dateWithS, dateByS) => {
    const response = await axiosPrivate.get(
      `${Urls.ANALYTICS_OFFLINE_PROFITABILITY}/filter/${dateWithF}/${dateByF}/${dateWithS}/${dateByS}`);
    await set({
